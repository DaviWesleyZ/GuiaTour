...
import com.example.android.guiatour.Activity.ComentariosActivity;
import com.example.android.guiatour.Activity.CompartilheActivity;
...
public class BlogRecyclerAdapter extends RecyclerView.Adapter<BlogRecyclerAdapter.ViewHolder>{
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        ...
        String thumbUri = blog_list.get(position).getImagem_thumb();
        ...
        firebaseFirestore.collection("Usuarios").document(user_id).get().addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
            ...
            public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                ...
                if(task.isSuccessful()){
                    ...
                    String userName = task.getResult().getString("nome");
                    String userImage = task.getResult().getString("imagem");
        ...
        firebaseFirestore.collection("Posts/" + blogPostId + "/Comentarios").addSnapshotListener(new EventListener<QuerySnapshot>() {
            @Override
            public void onEvent(QuerySnapshot documentSnapshots, FirebaseFirestoreException e) {
                if(!documentSnapshots.isEmpty()){

                    int count = documentSnapshots.size();
                    holder.updateComentariosCount(count);

                } else {

                    holder.updateComentariosCount(0);
    ...
    public class ViewHolder extends RecyclerView.ViewHolder{
        ...
        private TextView blogUserNome;
        private CircleImageView blogUserImagem;

        private ImageView blogLikeBtn, blogCommentBtn;
        private TextView blogLikeCount, blogComentariosCount;
        ...
        public void setUserData(String nome, String imagem){

            blogUserImagem = mView.findViewById(R.id.blog_user_image);
            blogUserNome = mView.findViewById(R.id.blog_user_name);

            blogUserNome.setText(nome);

            RequestOptions placeHolderOption = new RequestOptions();
            placeHolderOption.placeholder(R.drawable.profile_placeholder);

            Glide.with(context).applyDefaultRequestOptions(placeHolderOption).load(imagem).into(blogUserImagem);
        ...
        public void updateComentariosCount(int count) {
            blogComentariosCount = mView.findViewById(R.id.blog_comment_count);
            blogComentariosCount.setText(count + " Comentários");
        }
    }
}
